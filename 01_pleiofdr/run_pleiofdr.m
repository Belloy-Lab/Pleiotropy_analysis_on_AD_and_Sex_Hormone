% =============================================================================
% pleioFDR / conjFDR wrapper for Alzheimer's disease x sex hormone-related traits
%
% This wrapper only configures and runs pleioFDR. The pleioFDR implementation
% itself is NOT redistributed with this repository: install the upstream
% pleioFDR MATLAB functions separately and point the `mlibrary` option in the
% config file to that folder (for example the folder containing pleioOpt.m,
% TextConfig.m and the pleioFDR analysis functions).
%
% (c) 2019 University of Oslo - pleioFDR
% Pleiotropy-informed conditional and conjunctional false discovery rate
%
% =========================
% USER CONFIGURATION
% =========================
% Copy config_pleiofdr_template.txt to config_pleiofdr.txt and set:
%   traitfolder : folder with the pleioFDR-format .mat trait files
%   traitfile1  : .mat file of the primary trait (AD)
%   traitfiles  : .mat file(s) of the secondary trait(s) (hormones / negative controls)
%   traitnames  : matching trait names
%   reffile     : pleioFDR LD reference .mat file
%   mlibrary    : folder containing the upstream pleioFDR MATLAB functions
%   outputdir   : output folder
% The config file may be overridden by setting the variable `config` before
% running this script.
% =========================

clear all; close all; clc;

if ~exist('config', 'var'), config='config_pleiofdr.txt'; end
...
cfg.declare(CFG_TRAITFOLDER_STR,  'path/to/pleiofdr_mat_traits/');
cfg.declare(CFG_TRAITFILE1_STR, 'ad_female_zscore.mat');
cfg.declare(CFG_TRAITNAME1_STR, 'AD_female');
cfg.declare(CFG_TRAITFILES_CELL, '{''hormone_trait_zscore.mat''}');
cfg.declare(CFG_TRAITNAMES_CELL, '{''Hormone_trait''}');
cfg.declare(CFG_REFFILE_STR, 'path/to/reference/ref9545380_1kgPhase3eur_LDr2p1.mat');
cfg.declare(CFG_REFINFO_STR, '');
cfg.declare(CFG_MATLIBRARY_PATH_STR, 'path/to/pleiofdr_matlab_functions/');
cfg.declare(CFG_RANDPRUNE_BOOL, true);
cfg.declare(CFG_RANDPRUNE_GC_BOOL, true);
cfg.declare(CFG_RESET_PRUNEIDX_BOOL, true);
cfg.declare(CFG_RANDPRUNE_N_NUM, 500);
...
cfg.declare(CFG_OUTPUTDIR_STR, 'path/to/output/');
...
cfg.declare(CFG_MANH_YMARGIN_NUM, 0.5);
cfg.declare(CFG_MANCOLORLIST_MAT, ...
    '[1 0 0; 1 0.5 0 ; 0 0.75 0.75; 0 0.5 0; 0.75 0 0.75; 0 0 1; 0 1 0; 0 1 1]');
cfg.declare(CFG_EXCLUDE_CHR_POS_MAT, '[6 25119106 33854733; 8 7200000 12500000; 17 40000000 47000000]');
cfg.declare(CFG_EXCLUDE_FROM_DISCOVERY_BOOL, true);
cfg.declare(CFG_MAFTHRESH_NUM, 0.005);
cfg.declare(CFG_USE_STANDARD_GC_BOOL, false);
cfg.declare(CFG_PERFORM_GC_BOOL, true);
cfg.declare(CFG_EXCLUDE_AMBIGUOUS_SNPS_BOOL, true);
...
mlibrary = cfg.get_str(CFG_MATLIBRARY_PATH_STR);  % folder containing the pleioFDR MATLAB functions
...
%% Execute the pleiotropy analysis (upstream pleioFDR function)
% Requires the upstream pleioFDR MATLAB functions on the path (see `mlibrary`).
pleiotropy_analysis
