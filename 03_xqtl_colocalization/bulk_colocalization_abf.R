...
#
# Bulk colocalization driver (coloc.abf) for one QTL dataset identified by
# study / tissue / qtl_type. The script is sourced by xqtl_coloc_abf.R and expects
...
for (i in 1:nrow(all_suggestive_snps)) {
  ...
  if (study == "kosoy" | study == "eQTLgen" | study == "SingleBrain") {
    ...
    merged_updated <- merged_updated %>%
      filter(abs(A1FREQ - EAF) <= 0.1) %>%  # effect allele frequency agreement between AD and QTL
      dplyr::rename(CHR = CHR.x, BP = BP.x, AD_P = P, AD_EAF = A1FREQ, AD_BETA = BETA,
...
fwrite(final_results, file.path(output_dir, paste(study, tissue, qtl_type, "abf_hg38.csv", sep = "_")))

message("Completed QTL colocalization ABF analysis for: ", study, " ", tissue)
