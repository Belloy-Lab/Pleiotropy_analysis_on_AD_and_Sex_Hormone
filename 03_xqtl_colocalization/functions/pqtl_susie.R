...
#
# pQTL colocalization with coloc.susie.
#
# Inputs
...
#
# Returns one row per protein (gene) with PP0-PP4 from the credible-set pair with the
# largest PP4. If SuSiE hits an unrecoverable "prior variance unreasonably large" error
# (typically a data/LD mismatch), the gene is skipped.
...
pQTL_colocalization <- function(merged, all_suggestive_snps, name_dset) {
  ...
  for (gene_row in 1:nrow(colocalizing_genes)) {
    ...
    snp_file <- file.path(temp_dir, paste0(study, "_", tissue, "_pQTL_", discovery, "_", locus_index, ".txt"))
    fwrite(snps_for_ld, snp_file, sep = "\t", col.names = FALSE)

    file_ld <- paste("LD_matrix", study, tissue, "pQTL", discovery, locus_index, sep = "_")
    ...
    cs_length_S1 <- 0
    cs_length_S2 <- 0
    prior_variance_error <- FALSE  # flag to detect unrecoverable errors
    ...
    for (coverage in coverage_values) {
      tryCatch({
        ...
        if (grepl("estimated prior variance is unreasonably large", conditionMessage(e))) {
          prior_variance_error <<- TRUE
        }
      })

      if (prior_variance_error) {
        break
      }
    }
    ...
    if (prior_variance_error) {
      message("Prior variance error (data/LD mismatch) for: ", gene_n)
      next
    }
