...
#
# eQTL colocalization with coloc.abf.
#
# Inputs
...
# discovery.
#
# Returns one row per gene with the colocalization posterior probabilities PP0-PP4.
...
eQTL_colocalization <- function(merged, all_suggestive_snps, name_dset) {
  ...
  # Genes / features located in the analysis window (locus +/- 1 Mb)
  ...
  for (gene_row in 1:nrow(colocalizing_genes)) {
    ...
    # AD GWAS: case-control; QTL: quantitative; s is the case proportion
    D1 <- list(
