...
#
# mQTL (DNA methylation) colocalization with coloc.susie.
#
# Inputs
#   merged              harmonised AD x mQTL table for one locus with columns
#                       CHR, BP, SNP, ALLELE1, ALLELE0, AD_EAF, AD_BETA, AD_SE, AD_P,
#                       AD_N, QTL_EAF, QTL_BETA, QTL_SE, QTL_P, QTL_N, CpG, CpGpos, s
...
#
# Returns one row per CpG with PP0-PP4 from the credible-set pair with the largest PP4.

mQTL_colocalization <- function(merged, all_suggestive_snps, name_dset) {
  ...
  if (nrow(colocalizing_genes) == 0) {
    message("No CpGs in the analysis window for ", name_dset)
    return(NULL)
  }
  ...
  for (gene_row in 1:nrow(colocalizing_genes)) {
    ...
    snp_file <- file.path(temp_dir, paste0(study, "_", tissue, "_mQTL_", discovery, "_", locus_index, ".txt"))
    fwrite(snps_for_ld, snp_file, sep = "\t", col.names = FALSE)

    file_ld <- paste("LD_matrix", study, tissue, "mQTL", discovery, locus_index, sep = "_")
