...
#
# sQTL colocalization with coloc.susie.
#
# Inputs
...
#
# Returns one row per intron cluster (gene) with PP0-PP4. In this implementation the
# reported probabilities are the maximum across credible-set pairs.
...
sQTL_colocalization <- function(merged, all_suggestive_snps, name_dset) {
  ...
  for (gene_row in 1:nrow(colocalizing_genes)) {
    ...
    snp_file <- file.path(temp_dir, paste0(study, "_", tissue, "_sQTL_", discovery, "_", locus_index, ".txt"))
    fwrite(snps_for_ld, snp_file, sep = "\t", col.names = FALSE)

    file_ld <- paste("LD_matrix", study, tissue, "sQTL", discovery, locus_index, sep = "_")
    ...
    if (is.null(out$summary$PP.H4.abf)) {
      ...
      coloc_PP0 <- max(out$summary$PP.H0.abf, na.rm = TRUE)
      coloc_PP1 <- max(out$summary$PP.H1.abf, na.rm = TRUE)
      coloc_PP2 <- max(out$summary$PP.H2.abf, na.rm = TRUE)
      coloc_PP3 <- max(out$summary$PP.H3.abf, na.rm = TRUE)
      coloc_PP4 <- max(out$summary$PP.H4.abf, na.rm = TRUE)
