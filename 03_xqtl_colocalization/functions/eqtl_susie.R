...
#
# eQTL colocalization with coloc.susie.
#
# Inputs
#   merged              harmonised AD x eQTL table for one locus with columns
...
#
# Returns one row per gene with PP0-PP4 from the credible-set pair with the largest PP4.

eQTL_colocalization <- function(merged, all_suggestive_snps, name_dset) {
  ...
  for (gene_row in 1:nrow(colocalizing_genes)) {
    ...
    # LD matrix for the variants of this gene
    snps_for_ld <- list(merged_c$SNP)
    snp_file <- file.path(temp_dir, paste0(study, "_", tissue, "_eQTL_", discovery, "_", locus_index, ".txt"))
    fwrite(snps_for_ld, snp_file, sep = "\t", col.names = FALSE)

    file_ld <- paste("LD_matrix", study, tissue, "eQTL", discovery, locus_index, sep = "_")
    ...
    # coloc.susie input: AD (case-control) and QTL (quantitative)
    ...
    # Lower the coverage until both datasets have at least one credible set
    ...
    if (cs_length_S1 == 0 || cs_length_S2 == 0) {
      message("S1 or S2 has no credible set for: ", gene_n)
      next
    }
