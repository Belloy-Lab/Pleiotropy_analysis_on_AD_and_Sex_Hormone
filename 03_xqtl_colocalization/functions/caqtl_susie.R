...
#
# caQTL (chromatin accessibility) colocalization with coloc.susie.
#
# Inputs
#   merged              harmonised AD x caQTL table for one locus with columns
#                       CHR, BP, SNP, ALLELE1, ALLE0, AD_EAF, AD_BETA, AD_SE, AD_P,
...
#
# Returns one row per chromatin-accessibility peak with PP0-PP4 from the credible-set
# pair with the largest PP4.

caQTL_colocalization <- function(merged, all_suggestive_snps, name_dset) {
  ...
  for (gene_row in 1:nrow(colocalizing_genes)) {
    ...
    snp_file <- file.path(temp_dir, paste0(study, "_", tissue, "_caQTL_", discovery, "_", locus_index, ".txt"))
    fwrite(snps_for_ld, snp_file, sep = "\t", col.names = FALSE)

    file_ld <- paste("LD_matrix", study, tissue, "caQTL", discovery, locus_index, sep = "_")
